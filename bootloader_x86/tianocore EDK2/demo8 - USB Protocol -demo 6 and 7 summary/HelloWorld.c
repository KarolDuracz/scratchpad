	...
	for (UINTN i = 0; i < HandleCount; i++) {
		...
		for (UINT8 port = 0; port < NumPorts; port++) {
			...
			if (EFI_ERROR(Status)) {
				...
				// get descriptors 
				//ReadUsbDeviceDescriptor(SystemTable, usb2, port);
			}
		}
...
EFI_STATUS _ReadUsbDeviceDescriptor(IN EFI_SYSTEM_TABLE  *SystemTable)
{
	EFI_STATUS Status;
	EFI_USB_IO_PROTOCOL *usbio;
	EFI_USB_DEVICE_DESCRIPTOR DevDecs;
	UINTN HandleCount;
	EFI_HANDLE *HandleBuffer;
	
	// loate all usb devices that support EFI_USB_IO_PROTOCOL
	Status = gBS->LocateHandleBuffer(ByProtocol, &gEfiUsbIoProtocolGuid, NULL, &HandleCount, &HandleBuffer);
	if (EFI_ERROR(Status)) {
		// no USB device found
		_print(SystemTable, (UINT32)0x1FFF1FFF, 0, 0, 0, 0, 0);
		CpuDeadLoop();
	} else {
		// found - pass test
		_print(SystemTable, (UINT32)0x2FFF2FFF, 0, 0, 0, 0, 0);
	}
	
	// loop trhough all detected usb device
	for (UINTN i = 0; i < HandleCount; i++) {
		Status = gBS->HandleProtocol(HandleBuffer[i], &gEfiUsbIoProtocolGuid, (VOID **)&usbio);
		if (EFI_ERROR(Status)) {
			// failed to get USB_IO protocol for device
			_print(SystemTable, (UINT32)0x1FFF11FF, 0, 0, 0, 0, 0);
			CpuDeadLoop();
		} else {
			// found - pass test
			_print(SystemTable, (UINT32)0x2FFF22FF, 0, 0, 0, 0, 0);
		}
		
		Status = usbio->UsbGetDeviceDescriptor(usbio, &DevDecs);
		if (EFI_ERROR(Status)) {
			// failed to get descriptor
			_print(SystemTable, (UINT32)0x1FFF111F, 0, 0, 0, 0, 0);
			CpuDeadLoop();
		} 
		
		// print if succeed
		_print(SystemTable, (UINT32)i, 0, 0, 0, 0, 1);
		_print(SystemTable, (UINT32)DevDecs.IdVendor, 0, 0, 0, 0, 1);
		_print(SystemTable, (UINT32)DevDecs.IdProduct, 0, 0, 0, 0, 1);
		_print(SystemTable, (UINT32)DevDecs.DeviceClass, 0, 0, 0, 0, 0);
...
EFI_STATUS ReadUsbDeviceDescriptor(IN EFI_SYSTEM_TABLE  *SystemTable, EFI_USB2_HC_PROTOCOL *usb, UINT8 deviceAddress)
{
	...
	// real HW sequence, on Virtual Box works without power reset sequence
	// not working well - on virtual box this reset configuration probably and after that can't get descriptor
	...
	//UINT8 Buffer[18]; // usb device descriptor size
	//UINTN BufferSize = sizeof(Buffer);
	EFI_USB_DEVICE_REQUEST request;
	UINT32 timeout = 1000; // 1 sec
	UINTN dataLength = sizeof(EFI_USB_DEVICE_DESCRIPTOR);
	//UINTN dataLength;
	UINT32 result;
	//static EFI_USB2_HC_TRANSACTION_TRANSLATOR *translator = NULL;
	...
	//UINTN maxPacketLen = 8;
	
	Status = usb->ControlTransfer(usb, deviceAddress, 2, 64, &request, EfiUsbDataIn,
		&devDesc, &dataLength, timeout, NULL, &result);
	
	//request.RequestType = USB_REQ_TYPE_STANDARD | USB_TARGET_DEVICE | USB_REQ_TYPE_IN;
	//request.Request = USB_REQ_GET_DESCRIPTOR;
	//request.Value = (USB_DESC_TYPE_DEVICE << 8); // device descriptor
	//reuqest.Index = 0;
	//request.Length = sizeof(Buffer);
	
	//Status = usb->ControlTransfer(usb, deviceAddress, 0, 0, &request, EfiUsbDataIn, Buffer, &BufferSize, timeout);
	...
	if (EFI_ERROR(Status)) {
			...
			_print(SystemTable, (UINT32)devDesc.DeviceClass, 0, 0, 0, 0, 1);
			_print(SystemTable, (UINT32)result, 0, 0, 0, 0, 0);
			
			//UINT16 VenId = (Buffer[9] << 8) | Buffer[8];
			//UINT16 DevId = (Buffer[11] << 8) | Buffer[10];
			//_print(SystemTable, (UINT32)VenId, 0, 0, 0, 0, 1);
			//_print(SystemTable, (UINT32)DevId, 0, 0, 0, 0, 0);
		}
  ...
  )
{
	...
	_ReadUsbDeviceDescriptor(SystemTable);
