...
<br /><br />
I have a demo made in python that executes the program from OVMF ResetVcetor to the SEC phase. But I'm not posting it yet. This executes the program using the https://pypi.org/project/unicorn/ library. You know the state of the registers, you know how the jumps are executed. You don't have to GUESS the program execution, it just goes through the entire CPU setup phase between the reset vector and the Sec. And the output here is https://github.com/tianocore/edk2/blob/master/OvmfPkg/ResetVector/Main.asm#L134, i.e. ```jmp rsi``` and, as the description shows, ```Jump to the 64-bit SEC entry point```. And it shows well in the registers. But I need to refine it. This helps better understand the sequence between RESET VECTOR and SEC. But GDB is also good and step into
