/** UiAppGui.c
  ...
  - Smoke particle demo
  - Calls external UefiMain3(...) (your HelloBootServicesTest)
**/
...
{
  if (r == NULL) return;
  UINT64 pc = GetPerformanceCounter();
  r->state = (UINT32)(pc ^ (pc >> 32) ^ 0xA5A5A5A5);
...
STATIC VOID SmokeUpdateAndRender(SMOKE_SYSTEM *ss, UINT32 px, UINT32 py)
{
  ...
  for (UINTN i = 0; i < SMOKE_MAX_PARTICLES; ++i) {
    ...
    if (p->life > 0.0) {
      ...
      //INT32 cx = (INT32)round(p->x) - ox;
      //INT32 cy = (INT32)round(p->y) - oy;
...
EFI_STATUS EFIAPI UefiMain3(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  while (UiTop() != NULL) {
    ...
    // if current selection has creature context, draw preview creature at corner
    if (cur->Selected < cur->ItemCount && cur->Items[cur->Selected].Context == gCreature && gCreature != NULL) {
      UINT64 tick = GetPerformanceCounter();
      UINT64 ns = GetTimeInNanoSecond(tick);
      UINTN frame = (UINTN)((ns / (UI_FPS_MS * 1000000ULL)) % gCreature->FrameCount);
