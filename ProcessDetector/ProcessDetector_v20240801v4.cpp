...
#define NLISTSIZE 10000 // for debug and test set to 101 - is default val from ANSIC 6-6 ex lookup table / hash table
static struct nlist *hashlist[NLISTSIZE];

unsigned hash_list(char* s)
{
	unsigned hashval;
	for (hashval = 0; *s != '\0'; s++) {
		hashval = *s + 31 * hashval;
	}
	return hashval % NLISTSIZE;
}

struct nlist* lookup(char* s)
{
	struct nlist* np;
	for (np = hashlist[hash_list(s)]; np != NULL; np = np->next) {
...
struct nlist* install(char* name, char* defn, int start, int end, int scenario, std::string str)
{
	struct nlist* np;
	unsigned hashval;
	...
	if ((np = lookup(name)) == NULL) {
		...
		hashval = hash_list(name);
		np->next = hashlist[hashval];
		np->hash = hashval; 
		np->scenario = scenario;
		np->start = start;
		np->end = end;
		np->original = _strdup(str.c_str());
		np->time = 0;
		hashlist[hashval] = np;
	}
	else {
...
void concat_name(std::string str)
{
	...
	if ((current_hash != prev_hash)) {
		...
		for (int i = 0; i < HASHSIZE; i++) {
			//std::cout << (hashtab[i].lock_bit)<< std::endl;
			if (temp_buf_for_unique_hash[i] == 1) {
				vec_hash.push_back(i);
				//install((char*)"name", new_str);
				install(new_str, new_str, icc.start, icc.end, scenario, str);
				//
				/*if (hashtab[i].lock_bit == -1) {
					...
					sdb.lock_bit = 1;
					hashtab[i] = sdb;
					std::cout << " lock bit test : " << hashtab[i].hash << " | " << hashtab[i].name << std::endl;
				}*/
				//
				hh++;
			}
	...
	/*-----------*/
	//struct nlist* np = lookup((char *)"name");
	//printf("defn : %s \n", np->defn);
	...
	for (int i = 0; i < NLISTSIZE; i++) {
		...
		/*=============================*/
			int MINIMALISTIC_VERSION = 1; // 0 print long version / 1 minimal output to cmd
		/*=============================*/

				if (MINIMALISTIC_VERSION == 0) {
					printf("%d | %d %d | %d %u %s | [ ORIGINAL ] %s \n", hashlist[i], hashlist[i]->start, hashlist[i]->end, hashlist[i]->scenario,
						hashlist[i]->hash, hashlist[i]->name, hashlist[i]->original);
				}
				else if (MINIMALISTIC_VERSION == 1) {
					printf("%d %u %d %s \n", hashlist[i], hashlist[i]->hash, hashlist[i]->time, hashlist[i]->name);
				}
		}
		else {
			// do nothing but if you cut to 101 NLISTSIZE variable it's ready for tests
			// but default is 10.000 for production mode
			//printf("%d \n", hashlist[i]);
		}

	}

	printf(" how many k1 : %d \n", k1);
	printf("------------------------------------------------------ \n");

	//
	//
	//
	// update stast in struct nlist
	struct nlist* current;
	char* current_name = _strdup(str.c_str());
	//current = lookup(current_name);
	current = lookup(new_str);
	if (current != NULL) {
		current->time += 1;
	}
...
DWORD WINAPI func_th2(LPVOID lpParam)
{
	...
	while (1) {
		...
		//	// Ctrl + c stop program 
		//	for (int i = 0; i < HASHSIZE; i++) {
		//		if (hashtab[i].hash != NULL) {
		//			std::cout << hashtab[i].hash << " : " << hashtab[i].name << std::endl;
		//		}
		//	}
		//	counter2 = 0;
		//}
